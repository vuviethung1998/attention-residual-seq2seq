...
def load_dataset(seq_len, horizon, input_dim, output_dim, dataset, test_size, valid_size, verified_percentage):
    load_with_holiday_increase_dow = pd.read_csv('data/holiday_increase_month_dow_doy.csv', usecols=['power','holiday','dayOfWeek','increase','month','dayOfYear'] )
    raw_data = load_with_holiday_increase_dow[['holiday','dayOfWeek', 'month','power']]
